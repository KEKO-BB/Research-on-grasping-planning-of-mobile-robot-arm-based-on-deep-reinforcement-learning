...
class Mobile_Arm_Env(gym.Env):
    ...
    def step(self, action):
        # 将度数转换为弧度
        radian1 = math.radians(-75)
        radian2 = math.radians(-35)
        radian3 = math.radians(2)
        radian4 = math.radians(70)
        radian5 = math.radians(35)
        radian6 = math.radians(45)
        # 创建一个存储弧度的列表
        radians_list = [radian1, radian2, radian3, radian4, radian5, radian6]
        ...
        # 机械
        if -0.09 < self.last_robot_state[3] < -0.06:
            if self.last_robot_state[44] == 0:
                if self.last_robot_state[39] < 0:
                    Wt4 = 2.0 / (3 * abs(self.last_robot_state[3]) + 1)
                    Wt5 = 0
            elif self.last_robot_state[44] == 1:
                if self.last_robot_state[39] > 0:
                    Wt4 = 0
                    Wt5 = 2.0 / (3 * abs(self.last_robot_state[3]) + 1)

        if -0.09 < self.last_robot_state[6] < -0.06:
            if self.last_robot_state[45] == 0:
                if self.last_robot_state[40] < 0:
                    Wt4 = 2.0 / (3 * abs(self.last_robot_state[6]) + 1)
                    Wt5 = 0
            elif self.last_robot_state[45] == 1:
                if self.last_robot_state[40] > 0:
                    Wt4 = 0
                    Wt5 = 2.0 / (3 * abs(self.last_robot_state[6]) + 1)

        # # 小车底盘
        # if -0.2 < self.last_robot_state[3] < 0:
        #     Wt6 = 0.5 / (3 * abs(self.last_robot_state[3]) + 1)
        # else:
        #     Wt6 = 0
        # if -0.2 < self.last_robot_state[6] < 0:
        #     Wt7 = 0.5 / (3 * abs(self.last_robot_state[6]) + 1)
        # else:
        #     Wt7 = 0
        ...
        derta_q[4] = radians_list[0] - self.Connectioner.robot_model.arm_current_joints_red[1]
        derta_q[5] = radians_list[1] - self.Connectioner.robot_model.arm_current_joints_red[2]
        derta_q[6] = radians_list[2] - self.Connectioner.robot_model.arm_current_joints_red[3]

        derta_q[7] = radians_list[3] - self.Connectioner.robot_model.arm_current_joints_red[1]
        derta_q[8] = radians_list[4] - self.Connectioner.robot_model.arm_current_joints_red[2]
        derta_q[9] = radians_list[5] - self.Connectioner.robot_model.arm_current_joints_red[3]

        ''' set action '''
        speed_action = self.speed_current + action[3]*self.dt
        # speed_action = self.speed_current + (
        #             Wt6 * (2.0 - self.speed_current) + Wt7 * (2.0 - self.speed_current) + action[3]) * self.dt
        arm_joint2_action = self.Connectioner.robot_model.arm_current_joints_red[1] + (
                    Wt1 * derta_q[1] + Wt4 * derta_q[4] + Wt5 * derta_q[7] + Wt2 * action[2]) * self.dt
        arm_joint3_action = self.Connectioner.robot_model.arm_current_joints_red[2] + (
                    Wt1 * derta_q[2] + Wt4 * derta_q[5] + Wt5 * derta_q[8] + Wt2 * action[1]) * self.dt
        arm_joint4_action = self.Connectioner.robot_model.arm_current_joints_red[3] + (
                    Wt1 * derta_q[3] + Wt4 * derta_q[6] + Wt5 * derta_q[9] + Wt2 * action[0]) * self.dt
        ...
        if -0.09 < robot_state[3] <= 0.05:
            # 定义安全距离阈值
            safe_distance = 0.13  # 假设关节与障碍物之间的安全距离为0.2米
            # 定义每个关节与障碍物距离的惩罚系数
            distance_penalty_coefficient = -40  # 当关节与障碍物的距离小于安全距离时的惩罚系数
            # 关节索引与其在状态向量中的对应位置
