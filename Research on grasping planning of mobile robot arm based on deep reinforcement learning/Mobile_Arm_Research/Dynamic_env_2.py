...
class Mobile_Arm_Env(gym.Env):
    ...
    def __init__(self, action_type='continuous') -> None:
        ...
        self.path = []  # 保存姿态路径
        ######################################
        self.last_robot_state = np.zeros(34)  #
        ...
        self.observation_space = spaces.Box(low=self.low, high=self.high, shape=(34,), dtype=np.float32)
    ...
    def get_current_state(self):
        ...
        # 获取关节速度
        _, _, arm_joint2_angular_velocity = vrep_sim.simxGetObjectVelocity(
            self.Connectioner.client_ID,
            self.Connectioner.robot_model.arm_joint_2,
            vrep_sim.simx_opmode_blocking
        )
        _, _, arm_joint3_angular_velocity = vrep_sim.simxGetObjectVelocity(
            self.Connectioner.client_ID,
            self.Connectioner.robot_model.arm_joint_3,
            vrep_sim.simx_opmode_blocking
        )
        _, _, arm_joint4_angular_velocity = vrep_sim.simxGetObjectVelocity(
            self.Connectioner.client_ID,
            self.Connectioner.robot_model.arm_joint_4,
            vrep_sim.simx_opmode_blocking
        )
        _, base_link_linear_velocity, _ = vrep_sim.simxGetObjectVelocity(
            self.Connectioner.client_ID,
            self.Connectioner.robot_model.base_link_handle,
            vrep_sim.simx_opmode_blocking
        )
        _, obstacle_linear_velocity, _ = vrep_sim.simxGetObjectVelocity(
            self.Connectioner.client_ID,
            self.Connectioner.robot_model.obstacle_handle,
            vrep_sim.simx_opmode_blocking
        )
        ...
        arm_joint4_horizontal_collison_dis = abs(
            math.sqrt(math.pow(arm_joint4_collison[1], 2) + math.pow(arm_joint4_collison[2], 2)))
        arm_joint3_horizontal_target_dis = abs(
            math.sqrt(math.pow(arm_joint3_target[1], 2) + math.pow(arm_joint3_target[2], 2)))
        arm_joint3_horizontal_collison_dis = abs(
            math.sqrt(math.pow(arm_joint3_collison[1], 2) + math.pow(arm_joint3_collison[2], 2)))

        # 到达目标点的标志
        if end_target_dis <= self.end_to_target_shortdis:
            reach_flag = 1

        robot_state = np.zeros(34)
        ...
        robot_state[16] = arm_joint4_angular_velocity[2]
        robot_state[17] = arm_joint3_target[0]
        robot_state[18] = arm_joint3_target[1]
        robot_state[19] = arm_joint3_target[2]
        robot_state[20] = arm_joint3_collison[0]
        robot_state[21] = arm_joint3_collison[1]
        robot_state[22] = arm_joint3_collison[2]
        robot_state[23] = arm_joint3_horizontal_target_dis
        robot_state[24] = arm_joint3_horizontal_collison_dis
        robot_state[25] = arm_joint3_angular_velocity[2]
        robot_state[26] = arm_joint2_angular_velocity[2]
        robot_state[27] = base_link_target[0]
        robot_state[28] = base_link_collision[0]
        robot_state[29] = base_link_linear_velocity[2]
        robot_state[30] = obstacle_linear_velocity[1]
        robot_state[31] = reach_flag
        robot_state[32] = collision_flag
        robot_state[33] = obstacle_flag
    ...
    def step(self, action):
        ...
        # Gt 的设计
        Wt1 = 0
        if self.last_robot_state[28] < 0:
            # Wt1=np.log(0.12*self.step_count+1)
            Wt1 = 0
        else:
            Wt1 = 3.0 / (3 * abs(self.last_robot_state[27]) + 1)
        ...
        if robot_state[32] > 0 :
             R_collision = -40
        # # 动作幅度奖惩
        # all_action = np.array([arm_joint2_action, arm_joint3_action, arm_joint4_action])  # 将列表转换为NumPy数组
        # Ra = -np.linalg.norm(all_action) ** 2  # 计算向量的范数平方
        ...
        # 该step的奖励
        reward = R_touch + R_time_step + R_end_to_target + R_collision
