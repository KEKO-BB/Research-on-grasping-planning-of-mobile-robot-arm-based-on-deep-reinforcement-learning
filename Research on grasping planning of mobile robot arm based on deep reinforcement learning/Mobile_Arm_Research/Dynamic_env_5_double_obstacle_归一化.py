...
class Mobile_Arm_Env(gym.Env):
    ...
    def __init__(self, action_type='continuous') -> None:
        ...
        self.path = []  # 保存姿态路径
        ######################################
        self.last_robot_state = np.zeros(45)  #
        ...
        self.observation_space = spaces.Box(low=self.low, high=self.high, shape=(45,), dtype=np.float32)
    ...
    def normalize_state(self, state):
        ...
        """
        # 归一化到 [-1, 1] 范围
        state_range = self.high - self.low
        return 2 * (state - self.low) / state_range - 1
    ...
    def get_current_state(self):
        ...
        robot_state[44] = obstacle_flag

        # 归一化状态
        normalized_state = self.normalize_state(robot_state)
        return normalized_state, arm_end
    ...
    def step(self, action):
        ...
        if self.last_robot_state[38] < 0:
            ...
            Wt1 = 3.0 / (3 * abs(self.last_robot_state[0]) + 1)
        Wt2 = 1
        Wt3 = 1
