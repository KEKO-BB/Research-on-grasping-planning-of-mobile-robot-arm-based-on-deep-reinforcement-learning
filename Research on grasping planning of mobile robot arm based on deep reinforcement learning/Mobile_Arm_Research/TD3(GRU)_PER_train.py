...
def get_args():
    ...
    # 优先经验回放的参数
    parser.add_argument('--alpha', default=0.7, type=float, help="alpha for PER")
    parser.add_argument('--beta', default=0.4, type=float, help="beta for PER")
...
def env_agent_config(cfg, seed=1):
    ...
    agent = TD3(n_states=n_states, n_actions=n_actions, cfg=cfg)  # 将GRU和PER参数传递给TD3类
    return env, agent
...
def train(cfg, env, agent):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while not done:
            ...
            agent.update(beta=cfg.beta)  # 更新时传递beta值
            state = next_state
