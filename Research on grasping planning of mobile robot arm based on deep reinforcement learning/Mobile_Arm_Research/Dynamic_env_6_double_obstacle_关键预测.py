...
class Mobile_Arm_Env(gym.Env):
    ...
    def __init__(self, action_type='continuous') -> None:
        ...
        self.path = []  # 保存姿态路径
        ######################################
        self.last_robot_state = np.zeros(121)  #
        ...
        # 定义状态空间
        self.low = np.array([0] * 121, dtype=np.float32)
        self.high = np.array([0] * 121, dtype=np.float32)
        ...
        self.history_positions = {
            'end_target': [],  # 存储机械臂末端相对于障碍物1的位置
            'end_collision': [],  # 存储机械臂末端相对于障碍物2的位置
            'arm_joint4_collision': [],  # 存储关节3相对于障碍物1的位置
            'arm_joint3_collision': [],  # 存储关节4相对于障碍物1的位置
            'base_link': [],
            'obstacle': [],
            'mark': []
        }
        self.history_length = 5  # 历史长度，例如存储过去5个时间步的位置
        # 假设每个历史位置增加了2个浮点数，总共6个历史位置
        extra_state_size = 15 * self.history_length
        state_size = 46 + extra_state_size  # 原始状态大小加上额外的历史信息大小
        #############################################################
        ...
        ################# 人工势能场法 ######################
        self.InverseKinematics_function = InverseKinematics.InverseKinematics_planning(L1=0.01351793110370636,
    ...
    def get_current_state(self):
        ...
        # # 预测未来的相对位置
        # future_end_obstacle1 = []
        # time_deltas = [0.1, 0.2, 0.5]  # 未来的时间点
        # for t in time_deltas:
        #     future_obstacle1_y = end_collision_1[1] + obstacle1_linear_velocity[1] * t
        #     future_y = -future_obstacle1_y
        #     future_z = end_collision_1[2]
        #     future_end_obstacle1.extend([future_y, future_z])

        # 更新历史位置信息
        self.update_history_positions(end_target_dis, end_collision1_horizontal_dis, end_collision2_horizontal_dis,
                                      arm_joint4_horizontal_collision1_dis,
                                      arm_joint4_horizontal_collision2_dis, arm_joint3_horizontal_collision1_dis,
                                      arm_joint3_horizontal_collision2_dis,
                                      base_link_target[0], base_link_collision_1[0], base_link_collision_2[0],
                                      base_speed, obstacle1_linear_velocity[1],
                                      obstacle2_linear_velocity[1], reach_flag, collision_flag)
        # 到达目标点的标志
        if end_target_dis <= self.end_to_target_shortdis:
            reach_flag = 1

        # 创建固定长度的状态向量
        state_size = 46 + 15 * self.history_length
    ...
    def update_history_positions(self, end_target_dis, end_collision1_horizontal_dis, end_collision2_horizontal_dis,
                                 arm_joint4_horizontal_collision1_dis, arm_joint4_horizontal_collision2_dis,
                                 arm_joint3_horizontal_collision1_dis, arm_joint3_horizontal_collision2_dis,
                                 base_link_target_x, base_link_collision_1_x, base_link_collision_2_x,
                                 base_speed, obstacle1_velocity_y, obstacle2_velocity_y, reach_flag, collision_flag):
        # 更新历史相对位置
        self.history_positions['end_target'].append([end_target_dis])
        self.history_positions['end_collision'].append([end_collision1_horizontal_dis, end_collision2_horizontal_dis])
        self.history_positions['arm_joint4_collision'].append(
            [arm_joint4_horizontal_collision1_dis, arm_joint4_horizontal_collision2_dis])
        self.history_positions['arm_joint3_collision'].append(
            [arm_joint3_horizontal_collision1_dis, arm_joint3_horizontal_collision2_dis])
        self.history_positions['base_link'].append(
            [base_link_target_x, base_link_collision_1_x, base_link_collision_2_x, base_speed])
        self.history_positions['obstacle'].append([obstacle1_velocity_y, obstacle2_velocity_y])
        self.history_positions['mark'].append([reach_flag, collision_flag])
    ...
    def step(self, action):
        ...
        R_end_to_target = -Katt * (1 - math.exp(-robot_state[9]))
        # 到达目标点奖励
        if robot_state[42] > 0:
            R_touch = 100
            done = True
        ...
        if self.step_count > 150:  # 不能超过120步
            done = True

        # 测试
        # print(Katt)
        # print("{}".format(robot_state[44]))
        # 该step的奖励
