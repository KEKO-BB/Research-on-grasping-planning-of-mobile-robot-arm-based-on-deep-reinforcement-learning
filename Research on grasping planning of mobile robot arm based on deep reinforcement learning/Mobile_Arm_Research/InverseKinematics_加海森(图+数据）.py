...
class InverseKinematics_planning():
    ...
    def path_planning(self):
        ...
        data = pd.DataFrame({
            ...
            'Time': smooth_times[:len(iter_times)],  # Ensure lengths match
            'Objective Value': smooth_function_values[:len(iter_times)]
        })
        data.to_csv(r'D:\Dynamic_Arm_project\DDPG\逆解图\ik_results.csv', index=False)
