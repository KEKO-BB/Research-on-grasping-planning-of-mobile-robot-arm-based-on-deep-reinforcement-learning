...
class Mobile_Arm_Env(gym.Env):
    ...
    def __init__(self, action_type='continuous') -> None:
        ...
        ##########################################
        self.end_path = []  # 保存姿态路径
        self.obstacle_path = []  # 保存姿态路径
        ######################################
        self.last_robot_state = np.zeros(30)  #
        ...
        self.observation_space = spaces.Box(low=self.low, high=self.high, shape=(30,), dtype=np.float32)
    ...
    def get_current_state(self):
        ...
        _, arm_joint4 = vrep_sim.simxGetObjectPosition(
            self.Connectioner.client_ID,
            self.Connectioner.robot_model.arm_joint_4,
            -1,
            vrep_sim.simx_opmode_blocking
        )
        _, obstacle = vrep_sim.simxGetObjectPosition(
            self.Connectioner.client_ID,
            self.Connectioner.robot_model.obstacle_handle,
            -1,
            vrep_sim.simx_opmode_blocking
        ...
        #### 测试 #############################################
        self.end_path.append(arm_end)
        self.obstacle_path.append(obstacle)
        #######################################################
        reach_flag = 0
        collision_flag = 0
        # obstacle_flag = 0
        ...
        robot_state[26] = obstacle_linear_velocity[1]
        robot_state[27] = self.speed_current
        robot_state[28] = reach_flag
        robot_state[29] = collision_flag
        # robot_state[30] = obstacle_flag
    ...
    def step(self, action):
        ...
        # 障碍物惩罚
        # 与障碍物距离惩罚
        obstacle_distance_penalty = 0
        if -0.14 < robot_state[3] <= 0:
            # 定义安全距离阈值
            safe_distance = 0.15  # 假设关节与障碍物之间的安全距离为0.2米
            # 定义每个关节与障碍物距离的惩罚系数
            distance_penalty_coefficient = -20  # 当关节与障碍物的距离小于安全距离时的惩罚系数
            # 关节索引与其在状态向量中的对应位置
            joint_distance_indices = {
                5: 7,  # 机械臂末端与障碍物的水平距离
                4: 15,  # 关节4与障碍物的水平距离在状态向量中的索引
                3: 23,  # 关节3与障碍物的水平距离在状态向量中的索引
            ...
            for joint, index in joint_distance_indices.items():
                ...
                if distance_to_obstacle < safe_distance:
                    ...
                    obstacle_distance_penalty += penalty
        # 与障碍物碰撞惩罚
        R_collision = 0  # 碰撞惩罚
        if robot_state[29] > 0:
            R_collision = -40
    ...
    def reset_simple(self):
        ...
        # 增加起始位置随机
        pos_new = [0, 0, 0]
        pos_new[0] = self.Connectioner.robot_model.base_link_position[0] + np.random.uniform(0, 0)
        pos_new[1] = self.Connectioner.robot_model.base_link_position[1]
        pos_new[2] = self.Connectioner.robot_model.base_link_position[2]
        vrep_sim.simxSetObjectPosition(
            self.Connectioner.robot_model.client_ID,
