...
def get_args():
    ...
    # TD3特有的参数
    parser.add_argument('--policy_noise', default=0.06, type=float, help="standard deviation of policy noise")
    parser.add_argument('--noise_clip', default=0.12, type=float, help="maximum absolute value of policy noise")
    ...
    # GRU相关参数
    parser.add_argument('--gru_dim', default=256, type=int, help="dimension of GRU hidden state")
    parser.add_argument('--gru_layers', default=1, type=int, help="number of layers in GRU")
    parser.add_argument('--time_step_features', default=15, type=int, help="number of features per time step in GRU")
...
def env_agent_config(cfg, seed=1):
    ...
    agent = TD3(n_states=n_states, n_actions=n_actions, cfg=cfg)  # 将GRU参数传递给TD3类
    return env, agent
