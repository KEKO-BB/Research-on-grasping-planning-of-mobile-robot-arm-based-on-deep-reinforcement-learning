...
from Dynamic_env_6_double_obstacle_预测 import Mobile_Arm_Env  # 目前env7，半成功， env8 成功（成功率觉低）
import datetime
...
from Normal_Ounoise import NormalizedActions, GaussianExploration
...
def get_args():
    ...
    parser.add_argument('--memory_capacity', default=12000, type=int, help="memory capacity")  # 经验池容量
    parser.add_argument('--batch_size', default=138, type=int)  # 批量取出经验的大小
    ...
    # LSTM
    parser.add_argument('--lstm_dim', default=128, type=int, help="dimension of LSTM hidden state")
    parser.add_argument('--lstm_layers', default=1, type=int, help="number of layers in LSTM")
    parser.add_argument('--time_step_features', default=44, type=int, help="number of features per time step in LSTM")
    # step3 解析参数
...
def test(cfg, env, agent):
    print("Start testing!")
    print(f'Env:{cfg.env_name}, Algorithm:{cfg.algo_name}, Device:{cfg.device}')
    # agent.actor.eval()  # 设置actor为评估模式
    # agent.critic.eval()  # 设置critic为评估模式
    # 初始化统计变量
    failed_both = 0
